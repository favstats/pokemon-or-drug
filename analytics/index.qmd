...
```{=html}
<div class='card mb-3' style='border-left: 4px solid #6A4C93;'>  <div class='card-body'>    <div class='d-flex justify-content-between align-items-start'>      <div>        <h6 class='card-subtitle mb-2 text-muted'>Games Today</h6>        <h2 class='card-title mb-1' id='games-today'>-</h2>                </div>      <div class='text-primary'>{{< iconify ph:calendar size=2em >}}</div>    </div>  </div></div>
```

```{=html}
<div class='card mb-3' style='border-left: 4px solid #FF6B6B;'>  <div class='card-body'>    <div class='d-flex justify-content-between align-items-start'>      <div>        <h6 class='card-subtitle mb-2 text-muted'>Games Last Hour</h6>        <h2 class='card-title mb-1' id='games-last-hour'>-</h2>                </div>      <div class='text-primary'>{{< iconify ph:clock size=2em >}}</div>    </div>  </div></div>
```
...
(function() {
  ...
  function updateSummaryMetrics(data) {
    ...
    // Calculate games today
    const now = new Date();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const gamesToday = data.filter(d => {
      if (!d.Date) return false;
      const gameDate = new Date(d.Date);
      return gameDate >= todayStart;
    }).length;
    
    // Calculate games in last hour
    const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
    const gamesLastHour = data.filter(d => {
      if (!d.Date) return false;
      const gameDate = new Date(d.Date);
      return gameDate >= oneHourAgo;
    }).length;
    ...
    // Update games today and last hour
    const gamesTodayEl = document.getElementById('games-today');
    if (gamesTodayEl) {
      gamesTodayEl.textContent = gamesToday;
    }
    
    const gamesLastHourEl = document.getElementById('games-last-hour');
    if (gamesLastHourEl) {
      gamesLastHourEl.textContent = gamesLastHour;
    }
  }
